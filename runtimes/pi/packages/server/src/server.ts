...
export class PiServer {
	...
	private async failProtocol(connection: ConnectionState, error: ProtocolError): Promise<void> {
		...
		try {
			// Error frames must reach the client even when the configured
			// maxFrameLength is smaller than the error text, so encode them
			// against the protocol default instead of the inbound limit.
			finalFrame = encodeServerMessage(message, { maxFrameLength: DEFAULT_MAX_FRAME_LENGTH });
		} catch (encodeError) {
