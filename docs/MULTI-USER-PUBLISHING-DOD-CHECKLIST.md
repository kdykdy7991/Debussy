...
|---|---|---|
| 1. iframe 用公开 `publicAppId` 嵌入允许的宿主网页 | 完成 | `bootstrap`/`exchange` 公开定位 + Origin 白名单；`test/embed/embed-plane.test.ts`（bootstrap/exchange/dev turn 端到端）、`exchange.test.ts` |
...
| 12. 安全测试无未关闭 P0/P1 | 完成 | `docs/MULTI-USER-PUBLISHING-SECURITY-REVIEW.md`（10 项威胁全关闭）；`security.test.ts` |
| 13. 1,000 在线连接、30 并发文本 Turn 目标通过约定环境压测 | 30 并发 Turn 已验证；1,000 空闲 Realtime 待完整平面 | `docs/MULTI-USER-PUBLISHING-CAPACITY-REPORT.md`：30 并发 Turn p99≈55ms/0 错；1,000 空闲 Realtime + DB/Redis 短断需 Redis composed plane，列入报告「手工全平面压测」待发布前执行（不伪装） |
| 14. 宿主接入、运维、故障处理和回滚文档齐全 | 完成（本任务产出） | `HOST-INTEGRATION.md`、`OPS-RUNBOOK.md`、`mvp.env.example`、迁移/兼容说明 |

## MVP 线程闭环

- TASK-000～039 全部完成；服务端全量回归 **688 passed + 4 skipped（容量 env 门控）**。
- **禁止完成项**（TASK-039）核对：
