...
> 当前共同集成基线：`origin/main`（功能提交已包含至 `cefae75`，2026-08-25）。原 M0/M1
> 验收分支的统计、上下文、reasoning、发布 Runtime 与 Embed 实现均已进入主干。
> 这表示 M1 **功能代码已集成**，不表示 R1/M1 的正式发布门禁已全部通过；真实移动端、
> 完整异常场景、容量及备份恢复证据仍须补齐。
...
- [x] 前端和后端均从合并后的共同基线创建 M1 短期分支。备注：历史 M1 分支成果已完成集成，当前统一基线已推进到 `origin/main`。
- [x] 产品负责人确认允许进入 M1。备注：用户已指示前端开始 Metrics/Context adapter、页面骨架和状态壳；MCP 仍受 BE-3 门禁。

## C. M1：统计、模型参数与 Embed

> 进度快照（2026-08-25）：统计/上下文、reasoning 与 Embed 基础实现已进入
> `origin/main`。发布 Chat 后续完成了真实 Realtime upgrade、停止执行、会话隔离、
> 共享控制台 `ConversationWorkspace`/消息组件、真实 text/thinking 增量、终态持久化与
> 冻结 reasoning 默认解析。关键收口提交：`ac0918a`（共享 Workspace）、`5c39720`
>（流式语义一致）、`b1533a7`（发布 reasoning 默认）、`cefae75`（删除废弃 Web turn 路径）。
> 当前功能代码基线为 `cefae75`；最新验证为 Server 相关测试 55/55、Web 全量测试
> 462/462 及两包 TypeScript 检查通过。
>
> 已实际完成桌面 Chrome 的真实发布链接 + Qwen 对话验证，并确认发布 Chat 可显示
> 真实思考过程。尚未完成：完整真实浏览器 8 场景、Android Chrome/iOS Safari、
> 连续 ≥10 轮指标运行证据及 M1 总体验收。早期 API 前置证据和 runner 记录见
> [`evidence/m1-browser-acceptance-2026-08-24.md`](./evidence/m1-browser-acceptance-2026-08-24.md)。

### 当前阶段结论

- [x] M1 功能代码已合入 `origin/main`，不再停留在独立验收分支。
- [x] 管理员 Chat 与发布 Chat 的消息显示复用同一 `ConversationWorkspace` 和消息视图，不再维护仿制版渲染器。
- [x] 发布 Realtime 转发 Runtime 的真实正文/思考增量，完成事件持久化正文及可公开思考内容。
- [x] 发布 Runtime 按“会话覆盖 > Revision 显式参数 > 冻结 capability 默认”解析思考档位；不可关闭模型不再静默关闭思考。
- [x] 桌面 Chrome 已用真实发布链接和真实 Qwen 完成一次核心对话验证（人工确认正文与思考可见）。
- [ ] 完整桌面异常/恢复场景形成可复核验收记录。
- [ ] 真实 Android Chrome 与 iOS Safari 完成核心流程验收。
- [ ] 连续对话指标、20 并发文本对话、备份恢复和发布回滚演练形成正式证据。
- [ ] 产品负责人签署 M1 总体验收并决定是否进入 M2。
...
- [x] 后端向前端提供真实 API、五组 fixture、请求示例和特性开关。备注：reasoning 契约与 fixture 见 `reasoning-contract-and-integration.md`，前端生产路径已接真实 API。
...
- [ ] 在同一候选版本完成完整真实浏览器联调，记录请求 ID、会话 ID 和失败证据。备注：`cefae75` 已完成桌面 Chrome 核心成功路径人工验证，但尚未覆盖清单要求的全部异常/恢复场景及证据字段。
- [ ] 总架构师用可控 fake provider 验证 TTFT 与 tokens/s 公式。
- [x] 总架构师验证版本固定、参数白名单、Token 不落盘和错误 origin 拒绝。备注：自动化边界测试已覆盖；真实浏览器仍需作为发布验收复核，而非重新判定代码交付状态。
...
- [x] M1 前后端分支以可审查的小提交合并到共同验收基线并进入主干。备注：功能提交已包含至 `cefae75`。
- [ ] 产品负责人确认允许进入 M2。
