...
- [x] 后端提交 DTO、错误码、事件顺序和 `null`/旧数据语义。备注：第一轮候选已提交，未冻结。
- [x] 后端提交 reasoning 能力契约和 Provider 映射边界。备注：引用已完成规格，持久化问题待第二批候选。
...
- [x] 前端完成相关 UI/SDK 现有方案调研和最小验证。备注：第三轮已移除 `height == 0` skip，35 项 SDK 测试通过；仍需移除 SDK 重复高度边界、补 protocol 直接单测并纠正文档越界候选。
