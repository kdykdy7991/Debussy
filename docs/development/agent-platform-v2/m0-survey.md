...
| Agent 表单与思考编辑器 | [packages/web/src/admin/agents/agent-form.tsx](../../../../runtimes/pi/packages/web/src/admin/agents/agent-form.tsx) | `ModelParametersEditor` 已落地"只渲染 reasoning.{enabled,effort}"的契约；其余生成参数明确写"由服务端代码固定" |
...
| Reasoning 标签映射 | [packages/web/src/admin/agents/reasoning-efforts.ts](../../../../runtimes/pi/packages/web/src/admin/agents/reasoning-efforts.ts) | provider 枚举→UI 标签映射 |
...
| 模块 | 路径 | V2 复用点 |
|---|---|---|
| Embed SDK | [packages/web/src/embed/sdk/skdy-embed.ts](../../../../runtimes/pi/packages/web/src/embed/sdk/skdy-embed.ts) | V2 SDK 改造基线：`mount/destroy`、`ready`/`error`/`conversation-created`/`resize` 事件、origin 白名单、`launchToken` 一次性发送 |
...
| SessionController | [packages/web/src/lib/session-controller.ts](../../../../runtimes/pi/packages/web/src/lib/session-controller.ts) | `createSession/send/abort/setThinking/uploadFiles`；FE-1B 把 `setThinking` 改为会话级 `reasoningEffort` 覆盖 |
...
| 类型 | 路径 | V2 状态 |
|---|---|---|
| `AgentModelParameters` | [protocol/admin-workbench-agents.ts:89](../../../../runtimes/pi/packages/protocol/src/admin-workbench-agents.ts#L89) | 已冻结：仅 `reasoning.{enabled,effort}` |
| `ReasoningEffort` | [protocol/admin-workbench-agents.ts:87](../../../../runtimes/pi/packages/protocol/src/admin-workbench-agents.ts#L87) | `"minimal" \| "low" \| "medium" \| "high" \| "xhigh" \| "max"` |
| `LlmAvailableModel.parameterCapabilities.reasoning` | [protocol/admin-workbench-llm.ts:70](../../../../runtimes/pi/packages/protocol/src/admin-workbench-llm.ts#L70) | `supported / toggle / efforts / defaultEffort` |
...
| `EmbedHostPostMessage` / `EmbedIframePostMessage` | [protocol/embed/post-message.ts](../../../../runtimes/pi/packages/protocol/src/embed/post-message.ts) | v1 协议；含 `init`/`logout`/`focus`/`resize-request` 与 `ready`/`error`/`resize`/`conversation-created` |
| `POST_MESSAGE_LAUNCH_TOKEN_MAX_CHARS = 16384` | 同上 | SDK 端必须保留 |
| `POST_MESSAGE_RESIZE_MAX_HEIGHT = 100000` | 同上 | SDK 端必须保留 |
...
| 缺口 | 等 DTO？ | 备注 |
|---|---|---|
| 会话详情"上下文"区块 | 是 | 等 `ContextUsageSnapshot` |
| 会话详情"性能"区块 | 是 | 等 `TurnMetrics`；null 必须等于"未采集"，不得写0 |
| 会话级 `reasoningEffort` 覆盖的持久化端点 | 是 | 端点位置未定 |
| Agent 编辑页 `Skill Revision` 选择器 | 是 | 替换 `knowledgeBaseIds` 文本框 |
...
| 方案 | 优点 | 风险 | 推荐 |
|---|---|---|---|
| A. 仓库内独立 package `packages/embed-sdk`，新 workspace `@earendil-works/embed-sdk` | 包名/导出名稳定；与控制台示例天然一致；构建产物独立 | 锁文件变更需要审查 | 推荐 |
...
| 不确定点 | 验证内容 | 验收方式 |
|---|---|---|
| SDK 包名/导出名 | `import { createEmbed } from "@earendil-works/embed-sdk"` 与控制台 `app-detail.tsx` 示例一致 | 构建产物清单 + 最小 HTML 复刻运行 |
...
| resize 上限 | `height > POST_MESSAGE_RESIZE_MAX_HEIGHT` 必须裁剪/拒绝；恶意 Host 不出异常 | 单测覆盖 max+1 / NaN / 负数 |
| origin 校验 | 不在白名单的 origin 不更新 `targetOrigin`、不分发；错误 source 不分发 | 单测覆盖错误 source / origin / version |
| 多实例清理 | 两个独立实例互不串 listener；A.destroy() 不影响 B | 双实例 + 计数断言 |
...
| 思考强度覆盖 | `setThinking` 改为会话级 `reasoningEffort`；刷新/重连从 snapshot 恢复 | 单测 + 手动 e2e |

## 5. 工作顺序（与用户确认一致）

1. 本文档 + [contract-questions.md](./contract-questions.md) 提交推送。
2. 对 §4 中"不需要 DTO"的项做最小原型，单独 commit。
3. DTO 冻结方向明确后，进入页面骨架、状态壳与单一 fixture 适配层。