...
> **MVP-08 状态校正（2026-08）**：本 handoff 撰写时「用户会话」tab 仍是占位
> （"依赖 WB-006 用户会话列表 + WB-007 事件日志"），Launch Keys / Audit 是进入
> 详情时急切加载并静默吞错。MVP-06 已把「用户会话」tab 改为跳转
> `/conversations?appId=...`（列表页预填 appId 筛选）；将 Launch Keys / Audit
> 改为进入页签时惰性加载并显式报错；并补齐「接入方式」可复制 iframe/SDK 片段。
> 下方涉及这些占位/急切加载的表述已被 MVP-06 取代。仪表盘 / 八个页签骨架 /
> 发布抽屉 / 版本创建路径不变。
